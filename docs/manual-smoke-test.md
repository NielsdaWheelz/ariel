...
| `GET /v1/health` | Health | Returns runtime posture, schema revisions, capability digest, provider-evidence surface health, Google sync posture, and subscriber heartbeat when configured; returns 503 with schema issues when migrations are not ready. |
...
| `POST /v1/messages` | Direct user message | Enqueues a `user_message` task; worker completion creates the turn and optional Discord reply. |
| `GET /v1/events` | Timeline | Shows turns, events, approvals, and messages. |
...
| `POST /v1/messages` | `passed` | `partial` | Fixture-backed route enqueues a user-message task, rejects idempotency conflicts, drains the task, and replays the completed turn without a second model call. Live smoke posts a direct user message and observes the worker draining. Fixture anchor: `tests/integration/test_turn_lifecycle_acceptance.py::test_restart_preserves_history_and_appends_turns`. |
| `GET /v1/events` | `passed` | `passed` | Live smoke read the global timeline. |
...
| `cap.provider_evidence.read` | `provider_evidence.read` | Prior provider evidence ref from Gmail/calendar read or provider-sync context. | Rehydrate bounded provider evidence from Ariel's store without a Google call. |
...
| `cap.provider_evidence.read` | `passed` | `not_run` | DB-backed provider evidence read is fixture-proven; no fresh live provider-evidence rehydrate smoke was run in the current pass. Fixture anchors: `tests/integration/test_provider_evidence_read_runtime.py::test_provider_evidence_read_returns_text_to_program_and_redacts_audit` and `tests/integration/test_proactivity_scheduler.py::test_provider_sync_wake_requires_provider_evidence_read_before_message`. |
...
| Task replay | `passed` | `not_run` | Fixture anchor: `tests/integration/test_turn_lifecycle_acceptance.py::test_restart_preserves_history_and_appends_turns`. |
...
| Direct `user_message` task | `passed` | `partial` | Fixture anchor: `tests/integration/test_turn_lifecycle_acceptance.py::test_restart_preserves_history_and_appends_turns`; historical live route accepted a `user_message` task, but fresh completion against the current OpenRouter `MAIN` ref has not been recorded. |
...
| Memory dream | `passed` | `not_run` | Fixture anchors: `tests/integration/test_memory.py::test_worker_accepts_memory_encode_and_memory_dream` and `tests/integration/test_memory.py::test_worker_memory_dream_task_inserts_turn`. |
...
| `user_message` | `passed` | `partial` | Fixture anchor: `tests/integration/test_turn_lifecycle_acceptance.py::test_restart_preserves_history_and_appends_turns`; historical live route accepted the task, but fresh completion against the current OpenRouter `MAIN` ref has not been recorded. |
...
| `memory_encode` | `passed` | `passed` | Fixture anchors: `tests/integration/test_memory.py::test_memory_remember_enqueues_memory_encode_task` and `tests/integration/test_memory.py::test_memory_remember_enqueues_and_worker_records_encode_turn`; live worker drained a `memory_encode` task. |
| `memory_dream` | `passed` | `not_run` | Fixture anchor: `tests/integration/test_memory.py::test_worker_memory_dream_task_inserts_turn`. |
...
| Health | passed | `/v1/health` returns `ok: true` with runtime posture, schema revisions, provider-evidence surface health, a fresh `gmail_pubsub` heartbeat, and zero subscriber errors in window. |
