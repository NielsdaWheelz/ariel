...
- **goal**: implement the Slice 1 conversation contract so each turn deterministically takes one path (direct answer or clarifying question) using bounded recent in-session context.
...
- **acceptance**:
  ...
  - when user intent is ambiguous or conflicting, Ariel asks one focused clarifying question instead of guessing.
  ...
  - turn events make the conversation decision path observable for each completed turn (direct-answer vs clarifying-question).
- **non-goals**: no tool/capability execution, approvals, cross-session memory retrieval, or budget-exhaustion failure semantics.
...
- **acceptance**:
  ...
  - behavioral and machine-readable contracts are normative (decision path, terminal status, error code, structured details); exact user-facing copy is non-normative and may evolve without contract changes.
  ...
  - event chains preserve observability of decision path and terminal outcome for both successful and bounded-failure turns.
