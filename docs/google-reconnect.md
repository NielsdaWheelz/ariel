...
curl -s -X POST http://127.0.0.1:8000/v1/connectors/google/reconnect | jq -r '.oauth.authorization_url'
# → open the URL in a browser, click Allow

# Step 2: Gmail send (compose + send are bundled in cap.email.send via its scope)
curl -s -X POST 'http://127.0.0.1:8000/v1/connectors/google/reconnect?capability_intent=cap.email.send' | jq -r '.oauth.authorization_url'
# → open the URL, Allow

# Step 3: Calendar write (create / update / respond)
curl -s -X POST 'http://127.0.0.1:8000/v1/connectors/google/reconnect?capability_intent=cap.calendar.create_event' | jq -r '.oauth.authorization_url'
# → open the URL, Allow

# Optional — only add if you actually want them, each is one extra prompt:
curl -s -X POST 'http://127.0.0.1:8000/v1/connectors/google/reconnect?capability_intent=cap.email.archive' | jq -r '.oauth.authorization_url'  # archive / trash / labels
curl -s -X POST 'http://127.0.0.1:8000/v1/connectors/google/reconnect?capability_intent=cap.email.draft'   | jq -r '.oauth.authorization_url'  # drafts
curl -s -X POST 'http://127.0.0.1:8000/v1/connectors/google/reconnect?capability_intent=cap.drive.share'   | jq -r '.oauth.authorization_url'  # Drive share
curl -s -X POST 'http://127.0.0.1:8000/v1/connectors/google/reconnect?capability_intent=cap.drive.read'    | jq -r '.oauth.authorization_url'  # Drive read content
```

After each `curl`, check the granted state:

```bash
curl -s http://127.0.0.1:8000/v1/connectors/google | jq '{status: .connector.status, email: .connector.account_email, scopes: .connector.granted_scopes}'
```
...
- **Granted scopes still don't include the new one after Allow** — verify the callback succeeded: check `curl http://127.0.0.1:8000/v1/connectors/google/events | jq '.events[0:5]'` for a recent `evt.connector.google.reconnect.succeeded`.
...
```bash
curl -s -X DELETE http://127.0.0.1:8000/v1/connectors/google | jq
curl -s -X POST  http://127.0.0.1:8000/v1/connectors/google/start | jq -r '.oauth.authorization_url'
```
