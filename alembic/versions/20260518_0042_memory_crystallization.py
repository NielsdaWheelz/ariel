...
Drops the 30 surviving ``memory_*`` tables of the assertion / conflict-set /
projection engine and replaces them with a flat ``memory_facts`` store and a
singleton ``memory_profile`` document, adds a per-session ``digest`` column, and
amends the ``ai_judgments`` and ``background_tasks`` type CHECK constraints to
the two new memory judgment and task types.

``memory_deletions`` is intentionally absent: the schema-consolidation effort
already dropped it in revision 20260517_0036, so this migration neither drops
nor recreates it.

Revision ID: 20260518_0042
Revises: 20260517_0041
...
# and are ordered dependents-first; the 11 trailing entries have no inbound
# memory FK. memory_assertions self-references and so drops as a unit.
# memory_deletions is not listed: revision 20260517_0036 already dropped it.
...
def _downgrade_policy_tables() -> None:
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        # memory_deletions is intentionally absent from this list: revision
        # 20260517_0036 narrowed ck_memory_version_canonical_table to drop it,
        # so the post-20260517_0041 state this downgrade restores omits it too.
        sa.CheckConstraint(
    ...
    # memory_deletions is not recreated here: revision 20260517_0036 owns its
    # drop, and its own downgrade recreates it.
